...
Сделал Makefile для запуска всего, что понаписал. `make` - запустить все скрипты (сбилдить XS). Либо выборочно тоже можно, `make test1 [test2..test11]`
Ну и ниже частично продублировал ответы.
